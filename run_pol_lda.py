...
nlp = NlPipe.NlPipe(texts, path=path, document_ids=thread_ids, no_processes=15)
filter_array = np.logical_and(stat_df.language == 'en',
                              stat_df.replies > 10)
nlp.preprocess(load_existing=True, filter_loaded=filter_array)
nlp.create_bag_of_words(filter_extremes=False, use_phrases=None)
with threadpool_limits(limits=1, user_api='blas'):
    for max_df in tqdm([0.3, 0.2, 0.1], desc="max df"):
        ...
        nlp.search_best_model(topic_list=[25, 50, 100, 150, 200], passes=2,
                              alphas=['asymmetric', 0.01, 0.1, 0.3], etas=['auto', 0.01, 0.1, 0.3])